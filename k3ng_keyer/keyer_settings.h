...
#define eeprom_write_time_ms 30000
  ...
  #ifdef EXTRA_BUTTON
    #define number_of_non_memory_buttons 2  // command button + extra button
    // index of the extra button within button_array / analogbuttontemp - always the last button
    #define EXTRA_BUTTON_INDEX (analog_buttons_number_of_buttons - 1)
  #else
...
#define custom_startup_field "W7YFR"   // an example could be callsign and name, eg. "AB1XYZ Bob", (or "Worlds best operator" which requires a 20 column display), string length shouldo be no more than the number of columns on the display

...
#define FEATURE_TRAINING_PADDLE        // makes the word/callsign/QSO training tables available without FEATURE_TRAINING_COMMAND_LINE_INTERFACE, for paddle-only (no serial terminal) echo practice in Command Mode - required here since FEATURE_COMMAND_MODE_PROGRESSIVE_5_CHAR_ECHO_PRACTICE is enabled below without FEATURE_TRAINING_COMMAND_LINE_INTERFACE
